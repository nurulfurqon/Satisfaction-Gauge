<?php
	//buat class
	class C_user_pelanggan extends CI_Controller {
		
		
		function index($id=NULL){
			$this->load->helper(array('url','form'));
			$this->load->library(array('session','user_agent','form_validation','pagination'));
			$this->load->model('user/pelanggan/m_user_tampil_pelanggan','',TRUE);
			...
			//$this->form_validation->set_rules('txt_cari');
				...
				$config["base_url"]=base_url().'c_user_pelanggan/index';				
				$config["per_page"]=7;
				$config["total_rows"]=$n;
				...
				$data["tampil"]=$this->m_user_tampil_pelanggan->tampil($config['per_page'],$id);
				$data["no"]=$id;
			...
			$this->load->view('user/pelanggan/v_user_tampil_pelanggan',$data);
		}

		public function tambah()
		{
			$this->load->helper('form');
			$this->load->model('user/pelanggan/m_user_tambah_pelanggan','',TRUE);	
				...
				if($this->form_validation->run()==TRUE)
				{
					if(empty($nama)||empty($alamat)||empty($telepon)||empty($jenis_kelamin))
						...
						else
						{
							...
							$simpan = $this->m_user_tambah_pelanggan->simpandata($nama,$alamat,$telepon,$jenis_kelamin);
								if($simpan)
								{
			...
			$this->load->view('user/pelanggan/v_user_tambah_pelanggan',$data,$nama,$alamat,$telepon,$jenis_kelamin);
		}
		public function hapus()
			{
				$this->load->helper('form');
				$this->load->model('user/pelanggan/m_user_hapus_pelanggan','',TRUE);	
				$idnya = $this->uri->segment(3);
				//panggil model "hapusdata"
				$this->m_user_hapus_pelanggan->hapusdata($idnya);
				//pesan data sukses
		...
		{
			$this->load->helper('form');
			$this->load->model('user/pelanggan/m_user_detail_pelanggan','',TRUE);
			$idnya = $this->uri->segment(3);
			$data["pilih"] = $this->m_user_detail_pelanggan->detail_data($idnya);
			$this->load->view('user/pelanggan/v_user_detail_pelanggan',$data);
		}
		public function ubah()
		{
			$this->load->helper('form');
			$this->load->model('user/pelanggan/m_user_ubah_pelanggan','',TRUE);
			$this->load->library('form_validation');
			$idnya = $this->uri->segment(3);
			$data["pilih"] = $this->m_user_ubah_pelanggan->detail_data($idnya);
			...
			if($this->input->post('btn_ubah'))
			{
				...
				if($this->form_validation->run()==TRUE)
				{
					...
					else
					{
						...
						$ubah = $this->m_user_ubah_pelanggan->ubahdata($idnya,$nama,$alamat,$telepon);
						if($ubah)
						{
			...
			$this->load->view('user/pelanggan/v_user_detail_pelanggan',$data,$nama,$alamat,$telepon);
		}
	}
