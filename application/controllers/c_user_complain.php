<?php
	//buat class
	class C_user_complain extends CI_Controller {
		...
		function index($id=NULL){
			...
			$this->load->model('user/complain/m_user_tampil_complain','',TRUE);
			...
			$n = $query->num_rows();
				...
				$config['base_url'] = base_url().'index.php/c_user_complain/index/';
				$config['total_rows'] = $n;
				$config['per_page'] = '10';
				...
				$data['list_complain'] =$this->m_user_tampil_complain->tampil($config['per_page'],$id);
				$data["no"]=$id;
			...
			$this->load->view('user/complain/v_user_tampil_complain',$data);
		}

		public function tambah()
		{
			$this->load->helper(array('url','form'));
			$this->load->model('user/complain/m_user_tambah_complain','',TRUE);	
			...
			if($this->input->post('btn_simpan'))
			{
				...
				if($this->form_validation->run()==TRUE)
				{
					if(empty($pelanggan_id)||empty($pegawai_id)||empty($jenis_complain_id)||empty($tgl_complain)||empty($status))
						...
						{
							$simpan = $this->m_user_tambah_complain->simpandata($pelanggan_id,$pegawai_id,$jenis_complain_id,$tgl_complain,$status);
					...
					//alihkan ke halaman display
								redirect('c_user_complain');
							}
						}
				}
			}
			//tampil halaman simpan
			$data['tampil_data_pelanggan']=$this->m_user_tambah_complain->ambil_data_pelanggan();
			$data['tampil_data_pegawai']=$this->m_user_tambah_complain->ambil_data_pegawai();
			$data['tampil_data_jenis_complain']=$this->m_user_tambah_complain->ambil_data_jenis_complain();
			
			$this->load->view('user/complain/v_user_tambah_complain', $data,$pelanggan_id,$pegawai_id,$jenis_complain_id,$tgl_complain,$status);
		}
		public function hapus()
			{
				$this->load->helper('form');
				$this->load->model('user/complain/m_user_hapus_complain','',TRUE);	
				$idnya = $this->uri->segment(3);
				//panggil model "hapusdata"
				$this->m_user_hapus_complain->hapusdata($idnya);
				//pesan data sukses
		...
		{
			$this->load->helper('form');
			$this->load->model('user/complain/m_user_detail_complain','',TRUE);
			$this->load->library('form_validation');
			$idnya = $this->uri->segment(3);
			$data['tampil_data_pelanggan']=$this->m_user_detail_complain->ambil_data_pelanggan();
			$data['tampil_data_pegawai']=$this->m_user_detail_complain->ambil_data_pegawai();
			$data['tampil_data_jenis_complain']=$this->m_user_detail_complain->ambil_data_jenis_complain();
			$data["pilih"] = $this->m_user_detail_complain->detail_data($idnya);
				...
				if($this->form_validation->run()==TRUE)
					{
						...
						{
							$ubah = $this->m_user_detail_complain->ubah($idnya,$pelanggan,$pegawai,$jenis_complain,$tgl_complain,$status,$tgl_perbaikan);	
							if($ubah)
							{
			...
			$this->load->view('user/complain/v_user_detail_complain',$data,$pelanggan,$pegawai,$jenis_complain,$tgl_complain,$status,$tgl_perbaikan);
		}
		public function ubah()
		{
			$this->load->helper(array('url','form'));
			$this->load->model('user/complain/m_user_ubah_complain','',TRUE);
			...
			if($this->input->post('btn_ubah'))
			{
				...
				$this->m_user_ubah_complain->ubahdata($idnya);
				$this->session->set_flashdata('message', '<div class="alert alert-success"><span class="glyphicon glyphicon-ok"></span> Data berhasil diubah</div>');
				//alihkan ke halaman display
				redirect('c_user_complain');
