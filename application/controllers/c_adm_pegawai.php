<?php
	//buat class
	class C_adm_pegawai extends CI_Controller {
		
		function __construct(){
		parent::__construct();
			$this->load->model('admin/pegawai/m_adm_tampil_pegawai','',TRUE);
		}
		
		function index(){
			$data['tampil_data']=$this->m_adm_tampil_pegawai->ambil_data();
			$this->load->view('admin/pegawai/v_adm_tampil_pegawai',$data);
		}
		public function tambah()
		{
			$this->load->helper('form');
			$this->load->model('admin/pegawai/m_adm_tambah_pegawai','',TRUE);	
				...
				if($this->form_validation->run()==TRUE)
				{
					if(empty($nama)||empty($tahun_masuk)||empty($alamat)||empty($telepon)||empty($jenis_kelamin)||empty($type))
						...
						else
						{
							...
							$simpan = $this->m_adm_tambah_pegawai->simpandata($nama,$tahun_masuk,$alamat,$telepon,$jenis_kelamin,$type);
								if($simpan)
								{
			...
			$this->load->view('admin/pegawai/v_adm_tambah_pegawai',$data,$nama,$tahun_masuk,$alamat,$telepon,$jenis_kelamin,$type);
		}
		public function hapus()
			{
				$this->load->helper('form');
				$this->load->model('admin/pegawai/m_adm_hapus_pegawai','',TRUE);	
				...
				$this->m_adm_hapus_pegawai->hapusdata($idnya);
		...
		{
			$this->load->helper('form');
			$this->load->model('admin/pegawai/m_adm_detail_pegawai','',TRUE);
			$idnya = $this->uri->segment(3);
			$data["pilih"] = $this->m_adm_detail_pegawai->detail_data($idnya);
			$this->load->view('admin/pegawai/v_adm_detail_pegawai',$data);
		}
		public function ubah()
		{
			$this->load->helper('form');
			$this->load->model('admin/pegawai/m_adm_ubah_pegawai','',TRUE);
			$this->load->library('form_validation');
			$idnya = $this->uri->segment(3);
			$data["pilih"] = $this->m_adm_ubah_pegawai->detail_data($idnya);
			...
			if($this->input->post('btn_ubah'))
			{
				...
				if($this->form_validation->run()==TRUE)
				{
					...
					else
					{
						...
						$ubah = $this->m_adm_ubah_pegawai->ubahdata($idnya,$nama,$alamat,$telepon,$jenis_kelamin,$type);
						if($ubah)
						{
			...
			$this->load->view('admin/pegawai/v_adm_detail_pegawai',$data,$nama,$alamat,$telepon,$jenis_kelamin,$type);
		}
	}
